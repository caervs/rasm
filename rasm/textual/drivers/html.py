"""
The HTML article writer and supporting objects
"""
...
<html>
  ...
  <script>
    ...
    function main() {
        $(".staticmath").each(function(index, obj) {
          MQ.StaticMath(obj);
        })
...
FIGURE_TEMPLATE = """    <div style="width:100%; border:0px; margin:0px; padding-left:25%;">
      <div id="{name}" style="width:100%; border:0px; margin:0px; padding:0px;">
         <img src="{name}.svg" style="height: 400px; width: 100%;" />
      </div>
    </div>
"""

FIGURE_TEMPLATE = """
      <div id="{name}" style="margin-right: auto; margin-left: auto; width: 800px;">
         <img src="{name}.svg" style="height: 400px; width: 100%;" />
      </div>
"""
...
class Driver(object):
    ...
    def _parse_math(self, text):
        # TODO horribly memory inefficient
        toret = ""
        in_math_mode = False
        parts = list(text.split("$"))
        for i, subtext in enumerate(parts):
            if i > 0 and parts[i - 1] and parts[i - 1][-1] == '\\':
                toret += subtext
            elif in_math_mode:
                toret += '<font class="staticmath">{}</font>'.format(subtext)
                in_math_mode = False
            else:
                toret += subtext
                in_math_mode = True
    ...
    def write_header(self, header):
        self.body += "<h{}>{}</h{}>\n".format(header.level,
                                              self._parse_math(header.text),
                                              header.level)

    def write_text_line(self, line):
        self.body += self._parse_math(line.text) + "\n"

    def add_figure(self, figure):
        self.body += FIGURE_TEMPLATE.replace("{name}", figure.name)

    def write_list_block(self, lst):
        lines = [r"<ul>"] + ["<li>{}</li>\n".format(self._parse_math(item)) for item in lst.items] + \
                [r"</ul>", ""]
        self.body += "\n".join(lines)

    def save(self):
        """
        Save the produced html file
        """
        html = "{}.html".format(self.path)
        with open(html, 'wb') as f:
            f.write(HTML_TEMPLATE.replace("{body}", self.body).encode("utf-8"))
